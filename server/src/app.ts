...
// Builds the Express app without binding a port, so app.test.ts can exercise
// real HTTP request/response handling (routing, JSON parsing, status codes,
// validation) via supertest without an actual server or network socket --
// index.ts is the only thing that ever calls app.listen.
